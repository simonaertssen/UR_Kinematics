...
class RobotSocket(socket.socket):
    '''
    DESCRIPTION: Robot socket connection class.
    :param pos_init: None
    :return s: None
    '''
    def __init__(self):
...
def isRotationMatrix(R):
    '''
    DESCRIPTION: Checks if R is a rotation matrix.
    :param pos_init: R - Rotation matrix
    :return Boolean: True if R is a rotation matrix
    '''
    Rt = np.transpose(R)
...
def rotationMatrixToEulerAngles(R):
    ...
    :param R: Rotation maxtrix
    :return [rx,ry,rz]: converted rotations in euler angles
    '''
...
def Angles2rotvec(theta1, theta2):
    '''
    DESCRIPTION: Convert vertical and horizontal angle to rotation vector for UR Robot to understand.
    :param theta1: Vertical-rotation in radians
    :param pitch: Horizontal-rotation in radians
    :return Rx: Rotation in X direction in euler angles
    :return Ry: Rotation in Y direction in euler angles
    :return Rz: Rotation in Z direction in euler angles
    '''
...
def rotate(origin, point, angle):
    '''
    DESCRIPTION: Rotate point around the origin-point determined by the angle.
    :param origin: 2D point [x,y]
    :param point: 2D point [x,y]
    :param angle: radians
    :return qx: rotated x-coordinate
    :return qx: rotated y-coordinate
    '''
    """
...
def getPosition_modbus(s = None):
    '''
    DESCRIPTION: Returns the current position of the robot via modbus connection.
    :param s: robot modbus connection
    :return: [X,Y,Z,RX,RY,RZ] - robots current position
    '''
    program_run = 0
...
def set_Light(s = None, Off=True):
    '''
    DESCRIPTION: Turn on/off robot I/O 0 -> Ligt .
    :param s: robot connection
    :return: None
    '''
    # Connect to robot
    ...
    if Off:
        print("Off")
        s.send(b'set_digital_out(0, False)' + b"\n")
    else:
        print("On")
        s.send(b'set_digital_out(0, True)' + b"\n")
...
def open_gripper(s = None):
    '''
    DESCRIPTION: Open robot gripper.
    :param s: robot connection
    :return: None
    '''
    # Connect to robot
...
def close_gripper(s = None):
    '''
    DESCRIPTION: Close robot gripper.
    :param s: robot connection
    :return: None
    '''
    # Connect to robot
...
def set_IO_PORT(s = None, port_number =None, on=None):
    '''
    ...
    :return: None
    '''
    if port_number is None or on is None :
        print("Port number or on is \"NONE\"")
        return

    if port_number > 8 or port_number < 0 or not isinstance(port_number, int):
        print("Port number value error")
        return
...
def robot_line_sweep( pick_point, view_point, cam_pos, temp, img, save_image, show_image, path, s = None, optimize = False, camera = None):
    '''
    DESCRIPTION: Perform line sweep scanning in front of a camera.
    :param pick_point: [x1,y1,x2,y2,_,_,_,_] initial pose in front of camera
    :param view_point: [x1,y1,x2,angle,angle_interval,pictures_amount, view_type] initial pose in front of camera
    :param cam_pos: [x,y,z,rx,ry,rz] initial pose in front of camera
    :param temp: angle in radians....
    :param img: src image of object from library
    :param save_image: boolean - True:save
    :param show_image: boolean - True:show
    :param path: string - where to save image
    :param s: robot socket connection
    :param optimize: boolean - True:optimize view in front of camera
    :param camera: camera socket connection
    :return: None
    '''

    #TODO: ER DET HER KUN TIL DEMO?
    ...
    final_position, _, y_offset = generate_pos(cam_pos, view_point, pick_point, img, temp, s, optimize, camera, line_sweep=True)
...
def robot_point_view( pick_point, view_point, cam_pos, temp, img, save_image, show_image, path, s = None, optimize = False, camera = None):
    '''
    DESCRIPTION: Perform point view in front of a camera.
    :param pick_point: [x1,y1,x2,y2,_,_,_,_] initial pose in front of camera
    :param view_point: [x1,y1,x2,angle,angle_interval,pictures_amount, view_type] initial pose in front of camera
    :param cam_pos: [x,y,z,rx,ry,rz] initial pose in front of camera
    :param temp: angle in radians....
...
def robot_point_rotation( pick_point, view_point, cam_pos, temp, img, save_image, show_image, path, s = None):
    '''
    DESCRIPTION: Perform point rotation scanning in front of a camera.
    :param pick_point: [x1,y1,x2,y2,_,_,_,_] initial pose in front of camera
    :param view_point: [x1,y1,x2,angle,angle_interval,pictures_amount, view_type] initial pose in front of camera
    :param cam_pos: [x,y,z,rx,ry,rz] initial pose in front of camera
    :param temp: angle in radians....
...
def generate_pos(cam_pos, view_point, pick_point, img, temp , s, optimize = False, camera = None, line_sweep = False):
    '''
    DESCRIPTION: Perform line sweep scanning in front of a camera.
    :param cam_pos: [x,y,z,rx,ry,rz] initial pose in front of camera
    :param view_point: [x1,y1,x2,angle,angle_interval,pictures_amount, view_type] initial pose in front of camera
    ...
    :param optimize: boolean - True:optimize view in front of camera
    :param camera: camera socket connection
    :return position:  [x,y,z,rx,ry,rz] - optimal position
    :reuturn opt_image: optimal image
    :return y_offset: offset in y-direction
    '''
    ...
    print(2)
    if line_sweep:
        length = math.sqrt(((pick_point[2] + pick_point[0]) / 2 - (view_point[2] + view_point[0]) / 2) ** 2 + ( (pick_point[3] + pick_point[1]) / 2 - (view_point[3] + view_point[1]) / 2) ** 2)
    ...
    if optimize:
        if line_sweep:
            ...
            position[1] = opt_position[1]
        position, opt_img, y_offset = optimize_view(s, position.copy(), angle_hoiz, angle_ver, camera, line_sweep)
        print("After:", position)
        if line_sweep:
            position[1] = y_coordinate
            print("After 2:", position)

    print("GET POSITION END")
    return position, opt_img, y_offset


def optimize_view(s, position, angle_h, angle_v, camera = None, line_sweep =False):
    '''
    DESCRIPTION: Perform line sweep scanning in front of a camera.

    :param s: robot socket connection
    :param position: [x,y,z,rx,ry,rz] - position subject to optimization
    :param angle_h: horizontal angle
    :param angle_v: vertical angle
    :param camera: camera socket connection
    :return opt_position:  [x,y,z,rx,ry,rz] - optimal position
    :reuturn opt_image: optimal image
    :return y_offset: offset in y-direction
    '''
    opt_position = position.copy()
    ...
    robot_movej(s, position)
    # Optimize exposure
    img_opt, _ = Im.optimizeExposure(camera)
    ...
    for offset in range(-5, 6):
        ...
        cv2.imwrite(rootdir+'images/temp/temp.png', img)
    ...
    # Optimize exposure
    img_opt, _ = Im.optimizeExposure(camera, opt_image)
    ...
    for offset in range(30):
        ...
        if temp > sharpness:
            ...
            opt_image = img.copy()
        elif temp*1.10 < sharpness and temp >0.6:
            break
    ...
    if not line_sweep:
        ...
        position[1] -= 0.005
        print("Optimize x axis")
        for offset in range(20):
            robot_movej(s, position)
            img = Im.get_image(F"21565643", camera)
            img_draw = img.copy()
            img_draw = cv2.putText(img_draw, "Optimizing X position", (10, img.shape[0] - 10), cv2.FONT_HERSHEY_SIMPLEX, 1, 255, 2, cv2.LINE_AA)
            cv2.imwrite(rootdir+'images/temp/temp_save.png', img_draw)
            cv2.imwrite(rootdir+'images/temp/temp.png', img_draw)
    ...
    time.sleep(0.2)
    if os.path.isfile(rootdir+'images/temp/temp_save.png'):
        os.remove(rootdir+'images/temp/temp_save.png')
